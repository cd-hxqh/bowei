...
public class OrderServiceAdapter extends RecyclerView.Adapter<OrderServiceAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        holder.relativeLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("orderservice",list.get(position));
    ...
    public void update(ArrayList<OrderService> data, boolean merge) {
        if (merge && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                OrderService orderService = list.get(i);
                boolean exist = false;
                for (int j = 0; j < data.size(); j++) {
                    if (data.get(j) == orderService) {
                        exist = true;
                        break;
                    }
                }
                if (exist) continue;
                data.add(orderService);
            }
        }
        list = data;
